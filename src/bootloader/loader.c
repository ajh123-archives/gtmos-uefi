...
Framebuffer* InitializeGOP(){
    ...
    if(EFI_ERROR(status)){
        Print(L"[FAIL] Unable to locate GOP\n\r");
        return NULL;
    }
...
EFI_STATUS efi_main (EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
    ...
    Print(L"Loading GTMOS kernel\n\r");

    EFI_FILE* Kernel = LoadFile(NULL, L"kernel.elf", ImageHandle, SystemTable);
    if (Kernel == NULL){
        Print(L"[FAIL] Could not load kernel\n\r");
    }
    else{
        Print(L"[OK] Kernel Loaded Successfully\n\r");
    }

    Print(L"Reading kernel header\n\r");
    Elf64_Ehdr header;
    {
        UINTN FileInfoSize;
    ...
    if (newFont == NULL){
        Print(L"[FAIL] Font is not valid or is not found\n\r");
    }
    else
    {
        Print(L"[OK] Font found. char size = %d\n\r", newFont->psf1_Header->charsize);
    }
